...
- `BL <date> (<heading words>)` = the `docs/build-log.md` entry with that
  date and heading.
- `RM:<line>` = `docs/roadmap.md` at that line, as of commit `509f94d`.
...
**Status words** are the project's own (RM:11-13): "implemented" means code
...
**Cut-off:** the build log's last entry (2026-09-24, build 3 archived). The
App Store submission of build 3 on 2026-09-24 is known from the owner's brief
for this document but **is not yet recorded** in the build log or roadmap;
it is listed once in §1 with that caveat and nowhere else.
...
| 2026-08-24 | "Make the MVP try-able" dropped; lean launch decided (public launch before streak mode). | RM:1847-1858 |
...
| 2026-09-08 → 09-10 | B-polish: D2 (one blue) closed 09-08, D1 (hybrid theme) closed 09-09, D5 and the app icon 09-10. | RM:2116-2276; BL 2026-09-08, 2026-09-09, 2026-09-10 |
| 2026-09-13 | Firebase project connected (Analytics + Crashlytics collecting). | RM:684-688 |
| 2026-09-14 | First run on a physical iPhone (every earlier check was simulator-only); bank account submitted to App Store Connect. | BL 2026-09-14; RM:1894-1900 |
| 2026-09-15 | Paid Apps Agreement Active; free-tier "Practice this" leak closed; avatar carousel and illustrated avatar set; Premium redesign batches 0–4. | RM:694-698; BL 2026-09-15 (all entries) |
...
| 2026-09-19 | Plan change: `monthly-climb-v2` becomes the launch branch; gamification ships in the first release. | RM:3-9 |
...
| 2026-09-22 | AI permission before Topic Practice; fixed first-day Daily Test; paywall moved to Home after the first climb; tomorrow's Daily Test prepared in the background; EU DSA trader verification Active. | BL 2026-09-22 (all entries); RM:746-747 |
| 2026-09-23 | `monthly-climb-v2` merged into `main`. | git `a0723c6` (the build log records only the opposite merge, `main` into the branch: BL 2026-09-23 (pre-merge cleanup …)) |
...
| 2026-09-24 | Per-question Daily Test explanation; Daily Test token budget measured and raised; proxy deployed; build 2 archived (never uploaded). | BL 2026-09-24 (Daily Test: a per-question explanation), (… output measured …), (proxy deployed; build 2 archived) |
| 2026-09-24 | Portrait lock on iPhone and iPad; build 3 (`1.0.0+3`) archived. | BL 2026-09-24 (portrait lock …) |
| 2026-09-24 | Build 3 submitted to App Store review. | Owner's brief for this document; **not yet recorded in build log or roadmap** |
...
| **First-day paywall moved to Home, after the climb, once.** | 2026-09-22 | The Day-0 result screen ended in a paywall card right after the first win, before the mountain moved. Hypothesis: the moment after a visible win converts better. | Paywall card at the end of the Day-0 result screen: removed. | Implemented, automated tests only; the 600 ms pause not device-confirmed. | BL 2026-09-22 (the first-day paywall moves to Home …); RM:310-320; PRD2 §13.14 |
...
| "Make the MVP try-able" (private distribution of the MVP). | 2026-08-24 | Seven people had already used the core loop in person; another small private round would repeat what is known. | RM:1847-1853 |
| Free launch (Free Apps Agreement, no bank account). | 2026-09-14 | Funnel work is one of the project's goals; shipping free defers it. | RM:1956-1960 |
...
| Pinning Restore Purchases / legal links to the bottom with `Expanded`. | [date not in record; paywall follow-up, see BL 2026-09-02] | Turned a short gap into a large void; reverted to one flowing list. | RM:1132-1138 |
...
| Ad-copy headline "Personalized feedback, not a feature list". | 2026-09-16 | Traced to no spec doc (`git log -S`: written as ad copy in `4d327b7`). | BL 2026-09-16 (Two checks …); RM:1800-1805 |
...
| Apple's "Monthly with a 12-Month Commitment" billing. | 2026-09-17 | Not part of the pricing decision; the disclosure block cannot state a commitment term. | BL 2026-09-17 (Savings-badge fix …); RM:741-745 |
...
| 2026-09-24 | 883 Flutter + 70 proxy at build 3 | BL 2026-09-24 (portrait lock …); RM:418-425 |
...
- ~$0.034 per Topic Practice session; 10 sessions/day ≈ $10.20/month against
  ≈ $3.54/month net annual revenue. RM:63-67; PRD2 §13.7, §13.8.
...
- The token log is deployed and accumulating; every unit-economics figure
  stays an estimate until weeks of traffic are measured. RM:124-131.
...
| Premium at 375x667, 2026-09-16 | footer top at 507 pt, plan cards' bottom at ~706–710 pt | RM:1825-1828 |
...
| App crashed opening the paywall. | Any `Purchases.*` call before `configure()` is a native `fatalError`, not a catchable Dart exception. | On-device verification on the simulator. | RM:1094-1116 |
...
| Premium vertical overlap. | Ordinary overflow tests only catch horizontal `RenderFlex` overflow. | On-device review. | BL 2026-09-16 (Two checks …); RM:1796-1812 |
...
|---|---|---|
| Premium comparison table does not show the daily session difference that the offer card promises. | Adding a row hides the plan cards at 375x667; fix by restructuring the table, not appending. | RM:45-49 |
| Daily Test sets repeat topics and scenarios. | Same prompt for everyone; revisit with shared Daily Test generation. | RM:50-54 |
| Shared Daily Test generation. | No users yet, no saving; decide after token-log data. | RM:35 |
| Unit economics unmeasured; token logs have no persistent storage. | Weeks of real traffic needed; Workers Analytics Engine recommended, not built. | RM:124-133; PRD2 §13.10 |
| GDPR Art. 27 EU representative not appointed. | Sole developer in Türkiye, EU not a primary market, pseudonymous data only. Revisit if EU users become material or EU marketing starts. | RM:761-767 |
| `hasFullAccess` swallows a RevenueCat failure and returns false. | Changing it changes the contract for every caller. | RM:356-359; BL 2026-09-23 (Premium prompt …) |
| Launch screen follows the system appearance, not the in-app theme. | iOS limitation noted as a known limit. | RM:333-335 |
| `LaunchImage` placeholder build warning. | Invisible over the background color; not a blocker. | RM:392-394 |
| AOT release binary not inspected for debug-tool strings; empty `debug_settings` table still created. | Removing the table needs a schema change; the iOS binary check was blocked at the time and not done since. | RM:88-95 |
| Premium plan cards at 375x667 need a scroll. | Only 393x852 was brought fully above the fold; to be checked on TestFlight. | RM:2073-2081, RM:2091-2092 |
| Spacing scale only partly adopted. | Foundation only; the rest of the app not migrated. | RM:2216-2219 |
| Developer-account membership address left incomplete. | Independent of DSA, not public, would need a certified translation for no benefit. | RM:753-756; BL 2026-09-16 (EU DSA …) |
...
| Out-of-scope launch items: mountain geometry, themes, final medal art, Home medal shortcut, v3 Home redesign, Turkish UI, theme toggle. | Each has its own recorded reason. | RM:26-37 |
| Pre-rewrite mirror backup `~/GrammarLens-backup.git`. | Kept until one week after launch, then deleted; never pushed. | RM:41-44 |
